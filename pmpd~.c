...
typedef struct _pmpd_tilde {
    ...
    struct _input *inM;
    struct _input *inDEnv;
    struct _inputLink *inD;
    struct _inputLink *inK;
    ...
    t_int nb_inlet, nb_outlet;
    t_int nb_inPos, nb_inForce, nb_inM, nb_inDEnv, nb_inD, nb_inK;
    t_int nb_outPos, nb_outSpeed;
...
t_int *pmpd_tilde_perform(t_int *w)
{
    ...
    for (si=0; si<n; si++)
    {
        for (loop=0; loop<x->nb_loop; loop++)
        {
            ...
            for (i=0; i<x->nb_inM; i++)
            {
                t_float M = x->inM[i].influence * x->inlet_vector[x->inM[i].nbr_inlet][si];
                x->inM[i].mass->invM = M>0 ? 1/M : 0;                
            }
            for (i=0; i<x->nb_inDEnv; i++)
                x->inDEnv[i].mass->D = x->inDEnv[i].influence * x->inlet_vector[x->inDEnv[i].nbr_inlet][si];                
            // compute forces generated by links (spring / dashpot)
            for (i=0; i<x->nb_inD; i++)
                x->inD[i].link->D = x->inD[i].influence * x->inlet_vector[x->inD[i].nbr_inlet][si];
            for (i=0; i<x->nb_inK; i++)
                x->inK[i].link->K = x->inK[i].influence * x->inlet_vector[x->inK[i].nbr_inlet][si];
            ...
            // compute new positions of masses
            // a mass does not move if M=0 (i.e : invM = 0)
            for (i=0; i<x->nb_mass; i++)
            {
                if (x->mass[i]->D && x->mass[i]->speedX) { // Amortissement en fonction de la vitesse
                    L = fabs(x->mass[i]->speedX);
                    F = -L * x->mass[i]->D;
...
void pmpd_tilde_bang(t_pmpd_tilde *x)
{
    ...
    for (i=0; i<x->nb_inPos; i++)    logpost(x, 2, "inPos (mass):%ld, Inlet:%ld, Mass:%ld, Amplitude:%f", i, x->inPos[i].nbr_inlet, x->inPos[i].mass->Id, x->inPos[i].influence);
    for (i=0; i<x->nb_inForce; i++)  logpost(x, 2, "inForce (mass):%ld, Inlet:%ld, Mass:%ld, Amplitude:%f", i, x->inForce[i].nbr_inlet, x->inForce[i].mass->Id, x->inForce[i].influence);
    for (i=0; i<x->nb_inM; i++)      logpost(x, 2, "inM (mass):%ld, Inlet:%ld, Mass:%ld, Amplitude:%f", i, x->inM[i].nbr_inlet, x->inM[i].mass->Id, x->inM[i].influence);
    for (i=0; i<x->nb_inDEnv; i++)   logpost(x, 2, "inDEnv (mass):%ld, Inlet:%ld, Mass:%ld, Amplitude:%f", i, x->inDEnv[i].nbr_inlet, x->inDEnv[i].mass->Id, x->inDEnv[i].influence);
    for (i=0; i<x->nb_inD; i++)      logpost(x, 2, "inD (link):%ld, Inlet:%ld, Link:%ld, Amplitude:%f", i, x->inD[i].nbr_inlet, x->inD[i].link->Id, x->inD[i].influence);
    for (i=0; i<x->nb_inK; i++)      logpost(x, 2, "inK (link):%ld, Inlet:%ld, Link:%ld, Amplitude:%f", i, x->inK[i].nbr_inlet, x->inK[i].link->Id, x->inK[i].influence);
    for (i=0; i<x->nb_outPos; i++)   logpost(x, 2, "outPos (mass):%ld, Outlet:%ld, Mass:%ld, Amplitude:%f", i, x->outPos[i].nbr_outlet, x->outPos[i].mass->Id, x->outPos[i].influence);
    for (i=0; i<x->nb_outSpeed; i++) logpost(x, 2, "outSpeed (mass):%ld, Outlet:%ld, Mass:%ld, Amplitude:%f", i, x->outSpeed[i].nbr_outlet, x->outSpeed[i].mass->Id, x->outSpeed[i].influence);
}
...
void pmpd_tilde_setDEnv(t_pmpd_tilde *x, t_float idx_mass, t_float D)
{
    if (!validate_index(x, (int)idx_mass, x->nb_mass, "mass")) return;
...
void pmpd_tilde_mass(t_pmpd_tilde *x, t_float M, t_float posX)
{
    ...
    struct _mass *new_mass = (struct _mass *)getbytes(sizeof(struct _mass));
    if (!new_mass) return;

    new_mass->invM = M > 0 ? 1 / M : 0;
    new_mass->speedX = 0;
    new_mass->posX = posX;
    new_mass->forceX = 0;
    new_mass->D = 0;
...
void pmpd_tilde_link(t_pmpd_tilde *x, t_float mass_1, t_float mass_2, t_float K, t_float D)
{
    // add a link
    // *mass1, *mass2, K, D;
    if (!validate_index(x, (int)mass_1, x->nb_mass, "mass1") ||
        !validate_index(x, (int)mass_2, x->nb_mass, "mass2")) return;
    
    struct _link *new_link = (struct _link *)getbytes(sizeof(struct _link));
    if (!new_link) return;
    
    new_link->mass1 = x->mass[(int)mass_1];
    new_link->mass2 = x->mass[(int)mass_2];
    new_link->K = K;
    new_link->D = D;
    new_link->Id = x->nb_link;

    x->link = (struct _link **)resizebytes(x->link, x->nb_link * sizeof(struct _link *), (x->nb_link + 1) * sizeof(struct _link *));
    if (!x->link) {
        freebytes(new_link, sizeof(struct _link));
        return;
    }

    x->link[x->nb_link] = new_link;
    x->nb_link++;
}
...
void pmpd_tilde_NLlink(t_pmpd_tilde *x, t_symbol *s, int argc, t_atom *argv)
{
    ...
    if (!validate_index(x, mass_1, x->nb_mass, "mass1") ||
        !validate_index(x, mass_2, x->nb_mass, "mass2")) return;

    struct _NLlink *new_NLlink = (struct _NLlink *)getbytes(sizeof(struct _NLlink));
    if (!new_NLlink) return;

    new_NLlink->mass1 = x->mass[mass_1];
    new_NLlink->mass2 = x->mass[mass_2];
    new_NLlink->K = (argc >= 3) ? atom_getfloatarg(2, argc, argv) : 0;
    new_NLlink->D = (argc >= 4) ? atom_getfloatarg(3, argc, argv) : 0;
    new_NLlink->Pow = (argc >= 5) ? atom_getfloatarg(4, argc, argv) : 1;
    new_NLlink->L0 = (argc >= 6) ? atom_getfloatarg(5, argc, argv) : 0;
    new_NLlink->Lmin = (argc >= 7) ? atom_getfloatarg(6, argc, argv) : -1000000;
    new_NLlink->Lmax = (argc >= 8) ? atom_getfloatarg(7, argc, argv) : 1000000;

    x->NLlink = (struct _NLlink **)resizebytes(x->NLlink, x->nb_NLlink * sizeof(struct _NLlink *), (x->nb_NLlink + 1) * sizeof(struct _NLlink *));
    if (!x->NLlink) {
        freebytes(new_NLlink, sizeof(struct _NLlink));
        return;
    }

    x->NLlink[x->nb_NLlink] = new_NLlink;
    x->nb_NLlink++;
}

void pmpd_tilde_inPos(t_pmpd_tilde *x, t_float idx_inlet, t_float idx_mass, t_float influence)
{
    if (!validate_index(x, idx_inlet, x->nb_inlet, "inlet") ||
        !validate_index(x, idx_mass, x->nb_mass, "mass")) return;
    x->inPos = (struct _input *)resizebytes(x->inPos, (x->nb_inPos) * sizeof(struct _input), (x->nb_inPos + 1) * sizeof(struct _input));
    if (!x->inPos) return;
...
void pmpd_tilde_inForce(t_pmpd_tilde *x, t_float idx_inlet, t_float idx_mass, t_float influence)
{
    if (!validate_index(x, idx_inlet, x->nb_inlet, "inlet") ||
        !validate_index(x, idx_mass, x->nb_mass, "mass")) return;
    x->inForce = (struct _input *)resizebytes(x->inForce, (x->nb_inForce) * sizeof(struct _input), (x->nb_inForce + 1) * sizeof(struct _input));
    if (!x->inForce) return;
...
void pmpd_tilde_inM(t_pmpd_tilde *x, t_float idx_inlet, t_float idx_mass, t_float influence)
{
    if (!validate_index(x, idx_inlet, x->nb_inlet, "inlet") ||
        !validate_index(x, idx_mass, x->nb_mass, "mass")) return;
    x->inM = (struct _input *)resizebytes(x->inM, (x->nb_inM) * sizeof(struct _input), (x->nb_inM + 1) * sizeof(struct _input));
    if (!x->inM) return;
    x->inM[x->nb_inM].nbr_inlet = (int)idx_inlet;
    x->inM[x->nb_inM].mass = x->mass[(int)idx_mass];
    x->inM[x->nb_inM].influence = influence;
    x->nb_inM++;
}

void pmpd_tilde_inDEnv(t_pmpd_tilde *x, t_float idx_inlet, t_float idx_mass, t_float influence)
{
    if (!validate_index(x, idx_inlet, x->nb_inlet, "inlet") ||
        !validate_index(x, idx_mass, x->nb_mass, "mass")) return;
    x->inDEnv = (struct _input *)resizebytes(x->inDEnv, (x->nb_inDEnv) * sizeof(struct _input), (x->nb_inDEnv + 1) * sizeof(struct _input));
    if (!x->inDEnv) return;
    x->inDEnv[x->nb_inDEnv].nbr_inlet = (int)idx_inlet;
    x->inDEnv[x->nb_inDEnv].mass = x->mass[(int)idx_mass];
    x->inDEnv[x->nb_inDEnv].influence = influence;
    x->nb_inDEnv++;
}

void pmpd_tilde_inD(t_pmpd_tilde *x, t_float idx_inlet, t_float idx_link, t_float influence)
{
    if (!validate_index(x, idx_inlet, x->nb_inlet, "inlet") ||
        !validate_index(x, idx_link, x->nb_link, "link")) return;
    x->inD = (struct _inputLink *)resizebytes(x->inD, (x->nb_inD) * sizeof(struct _inputLink), (x->nb_inD + 1) * sizeof(struct _inputLink));
    if (!x->inD) return;
    x->inD[x->nb_inD].nbr_inlet = (int)idx_inlet;
    x->inD[x->nb_inD].link = x->link[(int)idx_link];
    x->inD[x->nb_inD].influence = influence;
    x->nb_inD++;
}

void pmpd_tilde_inK(t_pmpd_tilde *x, t_float idx_inlet, t_float idx_link, t_float influence)
{
    if (!validate_index(x, idx_inlet, x->nb_inlet, "inlet") ||
        !validate_index(x, idx_link, x->nb_link, "link")) return;
    x->inK = (struct _inputLink *)resizebytes(x->inK, (x->nb_inK) * sizeof(struct _inputLink), (x->nb_inK + 1) * sizeof(struct _inputLink));
    if (!x->inK) return;
    x->inK[x->nb_inK].nbr_inlet = (int)idx_inlet;
    x->inK[x->nb_inK].link = x->link[(int)idx_link];
    x->inK[x->nb_inK].influence = influence;
    x->nb_inK++;
}

void pmpd_tilde_outPos(t_pmpd_tilde *x, t_float idx_outlet, t_float idx_mass, t_float influence)
{
    if (!validate_index(x, idx_outlet, x->nb_outlet, "outlet") ||
        !validate_index(x, idx_mass, x->nb_mass, "mass")) return;
    x->outPos = (struct _output *)resizebytes(x->outPos, (x->nb_outPos) * sizeof(struct _output), (x->nb_outPos + 1) * sizeof(struct _output));
    if (!x->outPos) return;
...
void pmpd_tilde_outSpeed(t_pmpd_tilde *x, t_float idx_outlet, t_float idx_mass, t_float influence)
{
    if (!validate_index(x, idx_outlet, x->nb_outlet, "outlet") ||
        !validate_index(x, idx_mass, x->nb_mass, "mass")) return;
    x->outSpeed = (struct _output *)resizebytes(x->outSpeed, (x->nb_outSpeed) * sizeof(struct _output), (x->nb_outSpeed + 1) * sizeof(struct _output));
    if (!x->outSpeed) return;
...
{
    if (x->mass) {
        for (int i = 0; i < x->nb_mass; i++) freebytes(x->mass[i], sizeof(struct _mass));
        freebytes(x->mass, x->nb_mass * sizeof(struct _mass *));
    }
    if (x->link) {
        for (int i = 0; i < x->nb_link; i++) freebytes(x->link[i], sizeof(struct _link));
        freebytes(x->link, x->nb_link * sizeof(struct _link *));
    }
    if (x->NLlink) {
        for (int i = 0; i < x->nb_NLlink; i++) freebytes(x->NLlink[i], sizeof(struct _NLlink));
    ...
    if (x->inM) freebytes(x->inM, x->nb_inM * sizeof(struct _input));
    if (x->inDEnv) freebytes(x->inDEnv, x->nb_inDEnv * sizeof(struct _input));
    if (x->inD) freebytes(x->inD, x->nb_inD * sizeof(struct _inputLink));
    if (x->inK) freebytes(x->inK, x->nb_inK * sizeof(struct _inputLink));
...
// multichannel handling copied from https://github.com/Spacechild1/vstplugin/blob/3f0ed8a800ea238bf204a2ead940b2d1324ac909/pd/src/vstplugin~.cpp#L4122-L4136
    ...
    class_addmethod(pmpd_tilde_class, (t_method)pmpd_tilde_inM, gensym("inM"), A_DEFFLOAT, A_DEFFLOAT, A_DEFFLOAT, 0);
    class_addmethod(pmpd_tilde_class, (t_method)pmpd_tilde_inDEnv, gensym("inDEnv"), A_DEFFLOAT, A_DEFFLOAT, A_DEFFLOAT, 0);
    class_addmethod(pmpd_tilde_class, (t_method)pmpd_tilde_inD, gensym("inD"), A_DEFFLOAT, A_DEFFLOAT, A_DEFFLOAT, 0);
    class_addmethod(pmpd_tilde_class, (t_method)pmpd_tilde_inK, gensym("inK"), A_DEFFLOAT, A_DEFFLOAT, A_DEFFLOAT, 0);
    ...
    class_addmethod(pmpd_tilde_class, (t_method)pmpd_tilde_setDEnv, gensym("setDEnv"), A_DEFFLOAT, A_DEFFLOAT, 0);
