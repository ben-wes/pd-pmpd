...
typedef struct _pmpd_tilde {
    ...
    t_int nb_inPos, nb_inForce, nb_inM, nb_inDEnv, nb_inD;
    ...
    for (si=0; si<n; si++)
    {
        for (loop=0; loop<x->nb_loop; loop++)
        {
            ...
            for (i=0; i<x->nb_inDEnv; i++)
                x->inDEnv[i].mass->D = x->inDEnv[i].influence * in[x->inDEnv[i].nbr_inlet][si];                
            // compute forces generated by links (spring / dashpot)
            for (i=0; i<x->nb_inD; i++)
                x->inD[i].link->D = x->inD[i].influence * in[x->inD[i].nbr_inlet][si];
...
void pmpd_tilde_bang(t_pmpd_tilde *x)
{
    ...
    for (i=0; i<x->nb_inM; i++)      logpost(x, 2, "inM:%ld, Inlet:%ld, Mass:%ld, Amplitude:%f", i, x->inM[i].nbr_inlet, x->inM[i].mass->Id, x->inM[i].influence);
    for (i=0; i<x->nb_inDEnv; i++)   logpost(x, 2, "inDEnv:%ld, Inlet:%ld, Mass:%ld, Amplitude:%f", i, x->inDEnv[i].nbr_inlet, x->inDEnv[i].mass->Id, x->inDEnv[i].influence);
    ...
    for (i=0; i<x->nb_inD; i++)      logpost(x, 2, "inD:%ld, Inlet:%ld, Link:%ld, Amplitude:%f", i, x->inD[i].nbr_inlet, x->inD[i].link->Id, x->inD[i].influence);
}
...
void pmpd_tilde_inD(t_pmpd_tilde *x, t_float idx_inlet, t_float idx_link, t_float influence)
{
    ...
    x->inD[x->nb_inD].link = x->link[(int)idx_link]; // Fix: properly reference the link
