...
t_int *pmpd2d_tilde_perform(t_int *w)
{
    ...
    t_int n = (t_int)(w[2]);
    t_float F, FX, FY, L, LX, LY, deltaL, invL, tempPosX, tempPosY;
    t_int i, si, loop;

    t_float sqSpeed, invSpeed;
    ...
    for (si = 0; si < n; si++) {
        for (loop = 0; loop < x->nb_loop; loop++) {
            ...
            // a mass does not move if M=0 (i.e : invM = 0)
                x->mass[i].speedX = x->mass[i].posX - x->mass[i].prevPosX;
                x->mass[i].speedY = x->mass[i].posY - x->mass[i].prevPosY;
                sqSpeed = x->mass[i].speedX*x->mass[i].speedX + x->mass[i].speedY*x->mass[i].speedY;
                if (x->mass[i].D && sqSpeed) { // velocity damping
                    x->mass[i].speed = sqrt(sqSpeed);
	            	invSpeed = 1 / x->mass[i].speed;
	            	F = -(x->mass[i].speed - x->mass[i].Doffset) * x->mass[i].D;
                ...
                // verlet integration
                tempPosX = x->mass[i].posX;
                tempPosY = x->mass[i].posY;
                x->mass[i].posX += x->mass[i].speedX;
                x->mass[i].posY += x->mass[i].speedY;
                x->mass[i].prevPosX = tempPosX;
                x->mass[i].prevPosY = tempPosY;
            }
        }
        ...
        // Ssnd vector value to the vector pointer and reset for next iteration
...
{
    t_int i;
    for (i=0; i<x->nb_mass; i++) logpost(x, 2, "mass:%ld, M:%f, posX:%f, posY:%f, DEnv:%f, DEnvOffset:%f",i, x->mass[i].invM<=0.?0:1/x->mass[i].invM/x->sr_mass_factor, x->mass[i].posX,x->mass[i].posY,x->mass[i].D,x->mass[i].Doffset);
...
{
    if (!validate_index(x, (int)idx_mass, x->nb_mass, "mass")) return;
    x->mass[(int)idx_mass].prevPosX += pos - x->mass[(int)idx_mass].posX;
...
{
    if (!validate_index(x, (int)idx_mass, x->nb_mass, "mass")) return;
    x->mass[(int)idx_mass].prevPosY += pos - x->mass[(int)idx_mass].posY;
...
// invM speedX posX force
{
    ...
    x->mass[x->nb_mass].speed = 0;
    x->mass[x->nb_mass].prevPosX = posX;
    x->mass[x->nb_mass].prevPosY = posY;
