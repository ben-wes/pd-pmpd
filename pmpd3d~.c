    ...
    for (si=0; si<n; si++)
    {
        for (loop=0; loop<x->nb_loop; loop++)
        {
            for (i=0; i<x->nb_inPosX; i++)
                ...
                x->inPosX[i].mass->posX = x->inPosX[i].influence * x->inlet_vector[x->inPosX[i].nbr_inlet][si];
            for (i=0; i<x->nb_inPosY; i++)
                x->inPosY[i].mass->posY = x->inPosY[i].influence * x->inlet_vector[x->inPosY[i].nbr_inlet][si];
            for (i=0; i<x->nb_inPosZ; i++)
                x->inPosZ[i].mass->posZ = x->inPosZ[i].influence * x->inlet_vector[x->inPosZ[i].nbr_inlet][si];
            for (i=0; i<x->nb_inForceX; i++)
                // get inlet value and make it a force to the specified mass
                x->inForceX[i].mass->forceX += x->inForceX[i].influence * x->inlet_vector[x->inForceX[i].nbr_inlet][si];
            for (i=0; i<x->nb_inForceY; i++)
                x->inForceY[i].mass->forceY += x->inForceY[i].influence * x->inlet_vector[x->inForceY[i].nbr_inlet][si];
            for (i=0; i<x->nb_inForceZ; i++)
                x->inForceZ[i].mass->forceZ += x->inForceZ[i].influence * x->inlet_vector[x->inForceZ[i].nbr_inlet][si];
...
void pmpd3d_tilde_reset(t_pmpd3d_tilde *x)
{
    ...
    post("factor: %f", x->sr_mass_factor);
    x->nb_link      = 0;
    x->nb_NLlink    = 0;
    x->nb_mass      = 0;
    x->nb_inPosX    = 0;
