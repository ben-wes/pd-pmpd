...
void pmpd3d_tilde_inPosX(t_pmpd3d_tilde *x, t_float idx_inlet, t_float idx_mass, t_float influence)
{
    ...
    x->inPosX = (struct _input *)resizebytes(x->inPosX, (x->nb_inPosX) * sizeof(struct _input), (x->nb_inPosX + 1) * sizeof(struct _input));
    if (!x->inPosX) return;
...
void pmpd3d_tilde_inPosY(t_pmpd3d_tilde *x, t_float idx_inlet, t_float idx_mass, t_float influence)
{
    if (!validate_index(x, idx_inlet, x->nb_inlet, "inlet") ||
        !validate_index(x, idx_mass, x->nb_mass, "mass")) return;
    x->inPosY = (struct _input *)resizebytes(x->inPosY, (x->nb_inPosY) * sizeof(struct _input), (x->nb_inPosY + 1) * sizeof(struct _input));
    if (!x->inPosY) return;
...
void pmpd3d_tilde_inPosZ(t_pmpd3d_tilde *x, t_float idx_inlet, t_float idx_mass, t_float influence)
{
    if (!validate_index(x, idx_inlet, x->nb_inlet, "inlet") ||
        !validate_index(x, idx_mass, x->nb_mass, "mass")) return;
    x->inPosZ = (struct _input *)resizebytes(x->inPosZ, (x->nb_inPosZ) * sizeof(struct _input), (x->nb_inPosZ + 1) * sizeof(struct _input));
    if (!x->inPosZ) return;
...
void pmpd3d_tilde_inForceX(t_pmpd3d_tilde *x, t_float idx_inlet, t_float idx_mass, t_float influence)
{
    if (!validate_index(x, idx_inlet, x->nb_inlet, "inlet") ||
        !validate_index(x, idx_mass, x->nb_mass, "mass")) return;
    x->inForceX = (struct _input *)resizebytes(x->inForceX, (x->nb_inForceX) * sizeof(struct _input), (x->nb_inForceX + 1) * sizeof(struct _input));
    if (!x->inForceX) return;
...
void pmpd3d_tilde_inForceY(t_pmpd3d_tilde *x, t_float idx_inlet, t_float idx_mass, t_float influence)
{
    if (!validate_index(x, idx_inlet, x->nb_inlet, "inlet") ||
        !validate_index(x, idx_mass, x->nb_mass, "mass")) return;
    x->inForceY = (struct _input *)resizebytes(x->inForceY, (x->nb_inForceY) * sizeof(struct _input), (x->nb_inForceY + 1) * sizeof(struct _input));
    if (!x->inForceY) return;
...
void pmpd3d_tilde_inForceZ(t_pmpd3d_tilde *x, t_float idx_inlet, t_float idx_mass, t_float influence)
{
    if (!validate_index(x, idx_inlet, x->nb_inlet, "inlet") ||
        !validate_index(x, idx_mass, x->nb_mass, "mass")) return;
    x->inForceZ = (struct _input *)resizebytes(x->inForceZ, (x->nb_inForceZ) * sizeof(struct _input), (x->nb_inForceZ + 1) * sizeof(struct _input));
    if (!x->inForceZ) return;
...
void pmpd3d_tilde_outPosX(t_pmpd3d_tilde *x, t_float idx_outlet, t_float idx_mass, t_float influence)
{
    if (!validate_index(x, idx_outlet, x->nb_outlet, "outlet") ||
        !validate_index(x, idx_mass, x->nb_mass, "mass")) return;
    x->outPosX = (struct _output *)resizebytes(x->outPosX, (x->nb_outPosX) * sizeof(struct _output), (x->nb_outPosX + 1) * sizeof(struct _output));
    if (!x->outPosX) return;
...
void pmpd3d_tilde_outPosY(t_pmpd3d_tilde *x, t_float idx_outlet, t_float idx_mass, t_float influence)
{
    if (!validate_index(x, idx_outlet, x->nb_outlet, "outlet") ||
        !validate_index(x, idx_mass, x->nb_mass, "mass")) return;
    x->outPosY = (struct _output *)resizebytes(x->outPosY, (x->nb_outPosY) * sizeof(struct _output), (x->nb_outPosY + 1) * sizeof(struct _output));
    if (!x->outPosY) return;
...
void pmpd3d_tilde_outPosZ(t_pmpd3d_tilde *x, t_float idx_outlet, t_float idx_mass, t_float influence)
{
    if (!validate_index(x, idx_outlet, x->nb_outlet, "outlet") ||
        !validate_index(x, idx_mass, x->nb_mass, "mass")) return;
    x->outPosZ = (struct _output *)resizebytes(x->outPosZ, (x->nb_outPosZ) * sizeof(struct _output), (x->nb_outPosZ + 1) * sizeof(struct _output));
    if (!x->outPosZ) return;
...
void pmpd3d_tilde_outSpeedX(t_pmpd3d_tilde *x, t_float idx_outlet, t_float idx_mass, t_float influence)
{
    if (!validate_index(x, idx_outlet, x->nb_outlet, "outlet") ||
        !validate_index(x, idx_mass, x->nb_mass, "mass")) return;
    x->outSpeedX = (struct _output *)resizebytes(x->outSpeedX, (x->nb_outSpeedX) * sizeof(struct _output), (x->nb_outSpeedX + 1) * sizeof(struct _output));
    if (!x->outSpeedX) return;
...
void pmpd3d_tilde_outSpeedY(t_pmpd3d_tilde *x, t_float idx_outlet, t_float idx_mass, t_float influence)
{
    if (!validate_index(x, idx_outlet, x->nb_outlet, "outlet") ||
        !validate_index(x, idx_mass, x->nb_mass, "mass")) return;
    x->outSpeedY = (struct _output *)resizebytes(x->outSpeedY, (x->nb_outSpeedY) * sizeof(struct _output), (x->nb_outSpeedY + 1) * sizeof(struct _output));
    if (!x->outSpeedY) return;
...
void pmpd3d_tilde_outSpeedZ(t_pmpd3d_tilde *x, t_float idx_outlet, t_float idx_mass, t_float influence)
{
    if (!validate_index(x, idx_outlet, x->nb_outlet, "outlet") ||
        !validate_index(x, idx_mass, x->nb_mass, "mass")) return;
    x->outSpeedZ = (struct _output *)resizebytes(x->outSpeedZ, (x->nb_outSpeedZ) * sizeof(struct _output), (x->nb_outSpeedZ + 1) * sizeof(struct _output));
    if (!x->outSpeedZ) return;
...
void *pmpd3d_tilde_new(t_symbol *s, int argc, t_atom *argv)
{
    ...
    while (argc && argv->a_type == A_SYMBOL) {
        if (atom_getsymbol(argv) == gensym("-m"))
            ...
            pd_error(x, "[pmpd3d~]: invalid argument");
        argc--, argv++;
    }
